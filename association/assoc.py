...
def assoc_day_rayleigh(date_path = '/Users/rishi/seis/RIS_icequakes/outputs/rayleigh_times/20150615'):
    ...
    for sta in sta_list:
        ...
        with open(sta_path, 'r') as f:
            lines = f.readlines()
            for line in lines:
                # Get the UTCDateTime from the line
                utc_time = line.strip()
                # Append the UTCDateTime to the list
                rayleigh_arrival_list.append([sta, utc_time, sta_coords['latitude'], sta_coords['longitude']])
    # Convert the list to a DataFrame
    df = pd.DataFrame(rayleigh_arrival_list, columns=['station', 'utc_time', 'latitude', 'longitude'])
    ...
    while i < N:
        ...
        if len(cluster_df) >= min_cluster_size and unique_stations >= min_cluster_stations:
